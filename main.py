...
def setup_periodic_tasks(sender, **kwargs):
    sender.add_periodic_task(
        crontab(hour=11, minute=0), daily_report.s(), name="Daily Report"
    )
    ...
    def sum_schedules(clocks):
        ...
        if isinstance(clocks, list):
            for clock in clocks:
                ...
                elif clock["type"] == "CronClock":
                    cronex = clock["cron"]
                    day_or = clock["day_or"]
                    start_time = now - timedelta(hours=24)
                    cron = croniter(cronex, start_time, day_or=day_or)
...
def get_active_flow_versions_yesterday(
    flows: pd.DataFrame, now: datetime
) -> pd.DataFrame:
...
def get_wrong_version_runs(
    flow_runs: pd.DataFrame, active_flow_versions_yesterday: pd.DataFrame
) -> pd.DataFrame:
    wrong_version_runs = {}
    agg_runs = flow_runs[
        ~flow_runs["state"].isin(["Scheduled", "Cancelled", "Submitted"])
    ]
    agg_runs = (
        agg_runs.groupby(by=["flow_id", "flow_name", "version"]).count().reset_index()
    )
    ...
    for flow_name in agg_runs["flow_name"].unique():
        ...
        if flow_name in list(active_flow_versions_yesterday["name"].unique()):
            active_versions_yesterday = set(
                active_flow_versions_yesterday[
                    active_flow_versions_yesterday["name"] == flow_name
                ].iloc[0, 1]
            )
            wrong_versions = ran_versions.difference(active_versions_yesterday)
            count = agg_runs[
                (agg_runs["flow_name"] == flow_name)
                & (agg_runs["version"].isin(wrong_versions))
            ]["count"].sum()
            ...
            wrong_version_runs[flow_name] = {}
            wrong_version_runs[flow_name]["wrong_versions"] = set(
                agg_runs[agg_runs["flow_name"] == flow_name]["version"]
            )
            wrong_version_runs[flow_name]["count"] = agg_runs[
                agg_runs["flow_name"] == flow_name
            ]["count"].sum()
    ...
    dicio = {
        "name": flows,
        "wrong_versions_sets": list(map(lambda x: x["wrong_versions"], values)),
        "wrong_versions_count": list(map(lambda x: x["count"], values)),
    }
...
def get_active_schedules(flows: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, row in df.iterrows():
        active_flows = flows[
            (flows["name"] == row["name"])
            & (flows["is_schedule_active"] == True)
            & (flows["archived"] == False)
        ]
            ...
            "embeds": [
                {
                    ...
                    "author": author,
                    "fields": [
                        {"name": ":partying_face: Nenhum problema hoje!", "value": ""}
                    ],
                }
            ],
        }
        ...
        "embeds": [
            {
                ...
                "fields": fields
                + [
                    {"name": ":link: Ver mais em:", "value": "http://prefect.dados.rio"}
                ],
            }
        ],
    }
...
def generate_daily_report(now: datetime, config: dict):
    ...
    wrong_version_runs = get_wrong_version_runs(
        flow_runs, active_flow_versions_yesterday
    )
    ...
    flows["versions"] = flows["versions"].apply(
        lambda x: x if isinstance(x, list) else []
    )

    flows = flows.merge(wrong_version_runs, how="left")
    flows["wrong_versions_sets"] = flows["wrong_versions_sets"].apply(
    ...
    flows_agg.loc[:, "failed_percentage"] = (
        flows_agg["failed_runs"] / flows_agg["executed_runs"]
    )
    flows_agg.loc[:, "not_executed"] = (
        flows_agg["total_expected_runs"] - flows_agg["executed_runs"]
    )
    flows_agg.loc[:, "not_executed_percentage"] = (
        flows_agg["not_executed"] / flows_agg["total_expected_runs"]
    )
    flows_agg.loc[
        ~np.isfinite(flows_agg["not_executed_percentage"]), "not_executed_percentage"
    ] = 0
    flows_agg["priority"] = None

    for i, row in flows_agg.iterrows():
        if (
            row["failed_percentage"] >= 0.2
            or row["executed_runs"] < row["total_expected_runs"]
        ):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        now = parser.parse(sys.argv[1]).astimezone(pytz.timezone("America/Sao_Paulo"))
        daily_report(now)
    else:
        daily_report()
